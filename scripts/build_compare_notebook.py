...
Loads the latest checkpoints (Phase A: ResNet-UNet transfer-learned; Phase B: cGAN-polished),
plots training curves across all runs, and renders side-by-side visual comparisons on validation
images.
...
"""))
    ...
    cells.append(md("## 4. Load best generators (Phase A vs Phase B)"))

    cells.append(code("""
def load_generator(path: Path) -> ResNetUNet:
...
phase_a = load_generator(phase_a_ckpt)
phase_b = load_generator(phase_b_ckpt)
...
Each row is one image. The four columns are:

| Col 1 | Col 2 | Col 3 | Col 4 |
|---|---|---|---|
| **INPUT** — the B&W image we feed the model | **GENERATED — Phase A** (model output, no GAN) | **GENERATED — Phase B** (cGAN-polished output) | **REAL** — the actual original color photo (ground truth) |

So the leftmost column is what the model sees; the two middle columns are what the model
...
"""))
    ...
    cells.append(code("""
@torch.no_grad()
def predict_rgb(model, l_tensor):
...
column_colors = ["#444444", "#1f77b4", "#d62728", "#2ca02c"]

for row, idx in enumerate(indices):
    l_tensor, ab_true = val_ds[idx]
    panels = [
        gray_rgb(l_tensor),
        predict_rgb(phase_a, l_tensor),
        predict_rgb(phase_b, l_tensor),
    ...
    for col, panel in enumerate(panels):
        ...
        # Label every row so the user never loses track of which column is which
...
@torch.no_grad()
def l1_on_ab(model, l_tensor, ab_true):
    l_in = l_tensor.unsqueeze(0).to(device)
    ab_pred = model(l_in).cpu().squeeze(0).clamp(-1, 1)
...
for idx in indices:
    l_tensor, ab_true = val_ds[idx]
    la = l1_on_ab(phase_a, l_tensor, ab_true)
    lb = l1_on_ab(phase_b, l_tensor, ab_true)
    rows.append({"idx": idx, "phase_a_l1": la, "phase_b_l1": lb, "delta_b_minus_a": lb - la})
